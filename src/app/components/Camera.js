...
function Camera() {
  ...
  function capturePhoto() {
    const video = webcamRef.current?.video; // => Falidasi data nya gaada dia akan menjdi  undefined
    const canvas = canvasRef.current;

    if (video || !canvas || video.videoWidth) {
      setErrorMessage("Kamera Belum siap, Coba tunggu sebentar broew!");
    }
  ...
  return (
    <div>
      <div className="relative w-full rounded-2xl overflow-hidden bg-black">
        <Webcam ref={webcamRef} audio={false} videoConstraints={videoConstrains} className={`w-full ${isPortrait ? "aspect-[9/16]" : "aspect-video"} object-cover`} mirrored screenshotFormat="image/jpeg" screenshotQuality={0.9} />

        {photoDataUrl && <img src={photoDataUrl} alt="capture" className="absolute inset-0 w-full h-full object-cover" />}
        ...
        <div className="absolute bottom-2 left-1/2 -translate-1/2 flex items-center gap-3">
          ...
          <form action={formAction} onSubmit={onSubmit}>
            ...
            <button type="submit" disabled={!photoDataUrl || isLoading} className={`px-4 h-14 rounded-xl text-white shadow transition ${!photoDataUrl || isLoading ? "bg-gray-400" : "bg-emerald-700 hover:bg-emerald-800"}`} title="Analisis & Ramalan">
